...
export default function EditCategoryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [isSlugManuallyEdited, setIsSlugManuallyEdited] = useState(true); // Don't auto-override slug on edit
  ...
  useEffect(() => {
    const fetchCategory = async () => {
      try {
        ...
        const existingCategory = categories.find((c: any) => c._id === id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/categories/${id}`, { method: "PUT", body: formData });
    ...
    >
      ...
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border/40 pb-6">
        <div className="flex items-center gap-4">
          <Link href="/admin/products?tab=categories" className="w-9 h-9 flex items-center justify-center border border-border/40 text-text-muted hover:text-[#222222] hover:bg-secondary/50 transition-colors bg-white">
            <ArrowLeft className="w-4 h-4" strokeWidth={1.5} />
          </Link>
          <div>
            <h2 className="text-[18px] uppercase tracking-widest font-bold text-[#222222]">Edit Category</h2>
        ...
        >
          <Save className="w-3.5 h-3.5" /> {loading ? "Updating..." : "Update Category"}
        </button>
      </header>

    ...
    >
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 border border-border/40 bg-white">
        ...
        <div className="p-8 lg:p-16 flex flex-col justify-center">
          <span className="text-[11px] font-bold uppercase tracking-widest text-[#5B7763] mb-4">
            {article.category}
          </span>
          <h2 className="text-[28px] lg:text-[40px] font-bold leading-tight text-[#222222] mb-6">
            {article.title}
          </h2>
...
export function AnimatedArticleGridItem({
  article,
  index,
}: {
  article: any;
  ...
  return (
    ...
    >
      <Link
        href={`/blog/${article.slug}`}
        className="block overflow-hidden relative aspect-[4/3] mb-6 border border-border/40"
      >
        <Image
          src={article.image}
          alt={article.title}
      ...
      <span className="text-[10px] font-bold uppercase tracking-widest text-[#5B7763] mb-3">
        {article.category}
      </span>
      <Link href={`/blog/${article.slug}`}>
        <h3 className="text-[18px] font-bold leading-tight text-[#222222] mb-3 group-hover:text-[#5B7763] transition-colors line-clamp-2">
          {article.title}
        </h3>
      </Link>
      <p className="text-[13px] leading-relaxed text-text-muted mb-6 line-clamp-3">
        {article.excerpt}
      </p>
      <div className="mt-auto flex items-center justify-between pt-4 border-t border-border/40">
        ...
        <Link
          href={`/blog/${article.slug}`}
          className="text-[10px] font-bold uppercase tracking-widest text-[#222222] hover:text-[#5B7763] transition-colors"
        >
          Read
        </Link>
      </div>

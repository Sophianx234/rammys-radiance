...
import { Plus as Add01Icon, Filter as FilterIcon, Loader2 as Loading03Icon, MoreHorizontal as MoreHorizontalIcon, Search as Search01Icon, Shield as Shield02Icon, Mail as Mail01Icon, UserCircle as UserCircleIcon, Check as Tick02Icon } from "lucide-react";
...
export default function ManageTeamClient({ data }: ManageTeamClientProps) {
  ...
  const [inviteRole, setInviteRole] = useState<"admin" | "manager" | "dispatcher">("manager");

  // Edit Role Modal State
  const [isEditRoleOpen, setIsEditRoleOpen] = useState(false);
  const [memberToEdit, setMemberToEdit] = useState<TeamMember | null>(null);
  const [newRole, setNewRole] = useState<"admin" | "manager" | "dispatcher">("manager");
  ...
  const handleToggleStatus = (userId: string, currentStatus: string) => {
    startTransition(async () => {
      ...
      const result = await toggleTeamAccountStatus(userId, isSuspended);
      if (result.success) {
